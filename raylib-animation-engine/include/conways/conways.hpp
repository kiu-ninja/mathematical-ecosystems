...
struct ConwaysSimulation: public Scene::Scene { using Scene::Scene;
    int frame_count = 0;
    ConwaysState* cs;
...
public:
    ...
    void setup() override {
        ...
        add_scene_after_last(Scene::Controllers::timed(state.cells.scale(4), 2.0f, 3.0f));

        add_scene_after_last(Scene::Builders::executor([=](){
            ...
            Scene::Builders::instant([=](){
                ...
                for (int i = -LG_HALF; i <= LG_HALF; i++) {
                    for (int j = -LG_HALF; j <= LG_HALF; j++) {
                        if (!(abs(i) <= 1 && abs(j) <= 1))
                            sb->add(state.life_grid(i, j)->animate_visibility(0));
                    }
                }
                return sb;
            }),
        ...
        add_scene_after_last(Scene::Controllers::timed(
            Scene::Builders::instant([=](){
                ...
                for (int i = 0; i < state.neighbors.size(); i++)
                    sb->add(state.arrows.get<Arrow>(i)->from(state.life_grid(0, 0))->to(state.neighbors.get<GridCell>(i))->appear());
        ...
        // Background scenes
        add_scene(Scene::Controllers::infinite(new ConwaysSimulation(&state)));
    }
